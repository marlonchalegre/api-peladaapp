    ...
    (let [pelada-id (misc/as-uuid (:id (th/decode-body (app (-> (mock/request :post "/api/peladas")
      ...
      (testing "Voting updates status"
        ...
        (let [resp (app (-> (mock/request :get (str "/api/peladas/" pelada-id "/voting-status")) auth-admin))
          ...
          (let [p1-status (first (filter #(= p1-id (misc/as-uuid (:player_id %))) (:voters body)))]
      ...
      (testing "Accessing results after voting window closes should succeed"
        ...
        (let [resp (app (-> (mock/request :get (str "/api/peladas/" pelada-id "/voting-results")) auth-admin))
          ...
          (let [mvp-admin (first (filter #(= p-admin-id (misc/as-uuid (:player_id %))) (:mvp body)))
                mvp-p2 (first (filter #(= p2-id (misc/as-uuid (:player_id %))) (:mvp body)))]
