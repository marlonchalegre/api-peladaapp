...
(deftest votes-and-normalization
  (let [{:keys [app db-file]} (th/make-app!)
    ...
    (let [register (app (-> (mock/request :post "/auth/register") (mock/json-body {:name "Admin" :email "admin@example.com" :password "p"})))
          login (app (-> (mock/request :post "/auth/login") (mock/json-body {:email "admin@example.com" :password "p"})))
      ...
      ;; list votes
      (let [resp (app (-> (mock/request :get "/api/peladas/1/votes") (mock/header "authorization" (str "Token " token))))
            body (decode-body resp)]
        (is (= 200 (:status resp)))
        (is (= 2 (count body))))

      ;; normalized score for player 1 should be avg(5,3)=4 => 8
      (let [resp (app (-> (mock/request :get "/api/peladas/1/players/1/normalized-score") (mock/header "authorization" (str "Token " token))))
            body (decode-body resp)]
        (is (= 200 (:status resp)))
        (is (= 8.0 (:score body))))

      ;; self vote should fail
      (let [resp (app (-> (mock/request :post "/api/votes") (mock/header "authorization" (str "Token " token)) (mock/json-body {:pelada_id 1 :voter_id 1 :target_id 1 :stars 4}))) ]
        (is (= 400 (:status resp)))))))
