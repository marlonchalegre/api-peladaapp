...
(s/defn list-peladas-by-user :- [s/Any]
  ...
  (let [one-day-ago (-> (Instant/now) (.minus (Duration/ofDays 1)) Timestamp/from)
        query (-> (h/select :p.* [:o.name :organization_name] [:a.status :user_attendance_status]
                            ...
                            [[:raw "(SELECT string_agg(preview.name, '|') FROM (SELECT u2.name AS name FROM \"Attendance\" at_p JOIN \"OrganizationPlayers\" op2 ON op2.id = at_p.player_id JOIN \"Users\" u2 ON u2.id = op2.user_id WHERE at_p.pelada_id = p.id AND at_p.status = 'confirmed' ORDER BY at_p.updated_at ASC LIMIT 4) preview)"] :confirmed_preview])
                  (h/from [:Peladas :p])
